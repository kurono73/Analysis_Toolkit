...
def draw_luxmeter_panel(layout, scene, context, _):
    ...
    row.prop(scene, "lux_meter_ev_compensation", text="")
    ...
    row.label(text=f"{scene.lux_meter_avg_lux:.2f} lx")
    
    op = row.operator("scene_analysis.copy_value", text="", icon='COPYDOWN', emboss=False)
    op.value_to_copy = f"{scene.lux_meter_avg_lux:.2f}"
    
    row = col.row(align=True)
    row.label(text=_("Min / Max") + ":")
    sub_row = row.row(align=True)
    sub_row.label(text=f"{scene.lux_meter_min_lux:.2f} lx")
    sub_row.label(text=f"/ {scene.lux_meter_max_lux:.2f} lx")
    
    if scene.lux_meter_results:
    ...
    sun_box = layout.box()
    row = sun_box.row()
    row.prop(scene, "lux_meter_sun_panel_expanded", icon="TRIA_DOWN" if scene.lux_meter_sun_panel_expanded else "TRIA_RIGHT", icon_only=True, emboss=False)
    row.label(text=_("Sun Correction"), icon='LIGHT_SUN')
    if scene.lux_meter_sun_panel_expanded:
        sun_box.prop(scene, "lux_meter_sun_object", text=_("Sun Object"))
        sun_box.prop(scene, "lux_meter_correction_sensor", text=_("Basis Sensor"))
        row = sun_box.row(align=True)
        row.label(text=_("Target Lux")); row.prop(scene, "lux_meter_target_lux", text="")
...
def draw_luxev_panel(layout, scene, context, _):
    ...
    col.prop(scene, "lux_meter_conv_ev", text="EV")
    row = col.row(align=True)
    row.alignment = 'CENTER'
    row.label(icon='SORT_DESC')
    row.label(icon='SORT_ASC')
    col.prop(scene, "lux_meter_conv_lux", text="Lux")
    ...
    box = layout.box()
    row = box.row()
    row.prop(scene, "lux_meter_ref_panel_expanded", 
             icon="TRIA_DOWN" if scene.lux_meter_ref_panel_expanded else "TRIA_RIGHT", 
             icon_only=True, emboss=False)
    row.label(text=_("EV / Lux Reference"), icon='BOOKMARKS')

    if scene.lux_meter_ref_panel_expanded:
        sub_box = box.box()
        ...
        split.label(text="Lux (Approx.)")
        split.label(text=_("Condition"))
        
        grid.separator()
        # Table Content
        ref_data = [
            ("16", "~164,000 lx", _("Snow on a sunny day")),
            ("15", "~82,000 lx", _("Full sunlight")),
            ("14", "~41,000 lx", _("Hazy, some clouds, sunny day")),
            ("13", "~20,000 lx", _("Cloudy")),
            ("12", "~10,000 lx", _("Overcast, shady areas, sunrise/sunset")),
            ("9-11", "1,300-5,100 lx", _("Blue hour")),
            ("8", "640 lx", _("Bright street/indoor light")),
            ("5-7", "80-320 lx", _("Indoor / Bright window light")),
            ("2-4", "10-40 lx", _("Dim window light")),
            ("-1-1", "1-5 lx", _("Dark morning/evening")),
...
def draw_ev_panel(layout, scene, context, _):
    ...
    row.label(text=aperture_label)
    
    if s.solve_target == 'APERTURE':
        row.label(text=f"{prefix}/{s.aperture:.1f}")
    else:
        row.prop(s, "aperture_preset", text="")
        if s.aperture_preset == 'CUSTOM':
            custom_row = layout.row()
            split = custom_row.split(factor=0.3)
            split.label(text="")
            split.prop(s, "aperture", text=f"{prefix}/")

    # --- shutter ---
    if s.mode == 'STILLS':
        row = layout.row(align=True)
        row.label(text=_("Speed (s)"))
        if s.solve_target == 'SHUTTER':
            row.label(text=utils.format_shutter_speed(s.shutter_speed))
        else:
        ...
        row = layout.row(align=True)
        row.label(text=_("Angle (°)"))
        if s.solve_target == 'SHUTTER':
             row.label(text=_( "{angle:.1f}°", angle=math.degrees(s.shutter_angle)))
        else:
            row.prop(s, "shutter_angle", text="")
        
        row = layout.row(align=True)
        row.label(text=_("Frame Rate"))
        row.prop(s, "fps", text="")
        
    # --- ISO ---
    row = layout.row(align=True)
    row.label(text="ISO")
    if s.solve_target == 'ISO':
        row.label(text=f"ISO {s.iso:.0f}")
    else:
        row.prop(s, "iso_preset", text="")
        if s.iso_preset == 'CUSTOM':
            custom_row = layout.row(); split = custom_row.split(factor=0.3)
            split.label(text="")
            split.prop(s, "iso", text="")
            
    # --- ND Filter ---
    row = layout.row(align=True)
    row.label(text=_("ND Filter"))
    if s.solve_target == 'ND':
        # Only display the number of stops if the calculation result is greater than 0.01
        if s.nd_filter_stops > 0.01:
            row.label(text=_( "{stops:.1f} Stops", stops=s.nd_filter_stops))
        # Otherwise (almost zero or negative), indicate 'Not Required'.
        else:
            row.label(text=_("Not Required"), icon='INFO')
    else:
        row.prop(s, "nd_filter_preset", text="")
        if s.nd_filter_preset == 'CUSTOM':
            custom_row = layout.row(); split = custom_row.split(factor=0.3)
            split.label(text="")
            split.prop(s, "nd_filter_stops", text="")

            
    # --- EV ---
    row = layout.row(align=True)
    row.label(text=_("Exposure Value (EV)"))
    if s.solve_target == 'EV':
        row.label(text=f"EV {s.ev_value:.1f}")
    else:
        sub = row.row(align=True)
        sub.prop(s, "ev_value", text="")
...
def draw_horizon_panel(layout, scene, context, _):
    ...
    layout.prop(scene, "horizon_ground_offset")
    layout.operator("scene_analysis.horizon_distance", text=_("Calculate Horizon Distance"), icon='PLAY')
    if scene.horizon_distance_m > 0:
        res_box = layout.box()
        res_box.label(text=_("Result"))
        res_box.label(text=_( "From Camera: {cam}", cam=scene.horizon_camera_name), icon='CAMERA_DATA')
        row = res_box.row(align=True)
        row.label(text=_("Height above offset") + ":")
        row.label(text=utils.format_distance(context, scene.horizon_camera_height))
        row = res_box.row(align=True)
        row.label(text=_("Distance") + ":")
        row.label(text=utils.format_distance(context, scene.horizon_distance_m))
        op = row.operator("scene_analysis.copy_value", text="", icon='COPYDOWN', emboss=False)
        op.value_to_copy = f"{scene.horizon_distance_m:.4f}"
...
def draw_parallax_panel(layout, scene, context, _):
    ...
    row_frames.operator("scene_analysis.parallax_set_frame_a", text="A", icon='DECORATE_KEYFRAME'); row_frames.prop(scene, "parallax_start_frame", text="")
    row_frames.separator(factor=1.0)
    row_frames.operator("scene_analysis.parallax_set_frame_b", text="B", icon='DECORATE_KEYFRAME'); row_frames.prop(scene, "parallax_end_frame", text="")
    layout.operator("scene_analysis.parallax_find_extremes", text=_("Auto-Detect Max Range"), icon='AUTO')
    row_pixels = layout.row(align=True)
    row_pixels.prop(scene, "parallax_pixel_shift", text=_("Allowed Pixel Shift"))
    layout.operator("scene_analysis.parallax_distance", text=_("Calculate Parallax Distance"), icon='PLAY')
    if scene.parallax_distance_m > 0:
        res_box = layout.box(); pixel_shift_display = scene.parallax_pixel_shift
        res_box.label(text=_( "From Camera: {cam}", cam=scene.parallax_camera_name), icon='CAMERA_DATA')
        res_box.label(text=_( "{:.1f} Pixel Parallax Distance:", pixel_shift_display))
        row = res_box.row(align=True)
        row.label(text=utils.format_distance(context, scene.parallax_distance_m))
        op = row.operator("scene_analysis.copy_value", text="", icon='COPYDOWN', emboss=False); op.value_to_copy = f"{scene.parallax_distance_m:.4f}"
...
def draw_shooting_distance_panel(layout, scene, context, _):
    ...
    row_mode.prop_enum(scene, "distance_target_mode", 'CURSOR'); row_mode.prop_enum(scene, "distance_target_mode", 'ACTIVE_OBJECT')
    layout.operator("scene_analysis.distance_to_target", text=_("Calculate Shooting Distance"), icon='PLAY')
    if scene.distance_to_target_m > 0:
        res_box = layout.box()
        res_box.label(text=_("Result"))
        res_box.label(text=_( "From Camera: {cam}", cam=scene.distance_camera_name), icon='CAMERA_DATA')
        row = res_box.row(align=True)
        row.label(text=_("Distance") + ":")
        row.label(text=utils.format_distance(context, scene.distance_to_target_m))
        op = row.operator("scene_analysis.copy_value", text="", icon='COPYDOWN', emboss=False)
        op.value_to_copy = f"{scene.distance_to_target_m:.4f}"

def draw_converter_panel(layout, scene, context, _):
    """Draws the Unit Converter panel"""
    col_imperial = layout.column(align=True)
    col_imperial.prop(scene, "conv_imperial_unit", text="")
    if scene.conv_imperial_unit == 'FT': col_imperial.prop(scene, "conv_feet", text="ft")
    elif scene.conv_imperial_unit == 'FT_IN':
        row = col_imperial.row(align=True); row.prop(scene, "conv_feet", text="ft"); row.prop(scene, "conv_inches", text="in")
    elif scene.conv_imperial_unit == 'IN': col_imperial.prop(scene, "conv_inches", text="in")
    ...
    col_metric.prop(scene, "conv_metric_unit", text="")
    col_metric.prop(scene, "conv_metric_val", text="")
...
def draw_speedometer_panel(layout, scene, context, _):
    ...
    if s.mode == 'INSTANT':
        if s.target_obj:
            ...
            row = res_box.row(align=True)
            row.label(text=_("Current Speed") + ":")
            row.label(text=f"{speed_val:.2f} {unit_label}")
        ...
        if s.max_speed_ms >= 0:
            ...
            row = res_box.row(align=True)
            row.label(text=_("Average Speed") + ":")
            row.label(text=f"{avg_val:.2f} {unit_label}")
            op = row.operator("scene_analysis.copy_value", text="", icon='COPYDOWN', emboss=False)
            op.value_to_copy = f"{avg_val:.4f}"
            
            row = res_box.row(align=True)
            row.label(text=_("Max Speed") + ":")
            row.label(text=f"{max_val:.2f} {unit_label}")
            op = row.operator("scene_analysis.copy_value", text="", icon='COPYDOWN', emboss=False)
            op.value_to_copy = f"{max_val:.4f}"
            
            row = res_box.row(align=True)
            row.label(text=_("Min Speed") + ":")
            row.label(text=f"{min_val:.2f} {unit_label}")
            op = row.operator("scene_analysis.copy_value", text="", icon='COPYDOWN', emboss=False)
            op.value_to_copy = f"{min_val:.4f}"


# --- Static Panel Class Definitions ---

class ANALYSIS_PT_luxmeter(bpy.types.Panel):
    bl_label = "Illuminance Meter"
    bl_idname = "ANALYSIS_PT_luxmeter"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_luxmeter_panel(self.layout, context.scene, context, utils.translate)

class ANALYSIS_PT_texeldensity(bpy.types.Panel):
    bl_label = "UV SS Resolution"
    bl_idname = "ANALYSIS_PT_texeldensity"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_texeldensity_panel(self.layout, context.scene, context, utils.translate)

class ANALYSIS_PT_luxev(bpy.types.Panel):
    bl_label = "EV Lux Converter"
    bl_idname = "ANALYSIS_PT_luxev"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_luxev_panel(self.layout, context.scene, context, utils.translate)

class ANALYSIS_PT_ev(bpy.types.Panel):
    bl_label = "EV Calculator"
    bl_idname = "ANALYSIS_PT_ev"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_ev_panel(self.layout, context.scene, context, utils.translate)

class ANALYSIS_PT_horizon(bpy.types.Panel):
    bl_label = "Horizon Distance"
    bl_idname = "ANALYSIS_PT_horizon"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_horizon_panel(self.layout, context.scene, context, utils.translate)

class ANALYSIS_PT_parallax(bpy.types.Panel):
    bl_label = "Parallax Distance"
    bl_idname = "ANALYSIS_PT_parallax"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_parallax_panel(self.layout, context.scene, context, utils.translate)

class ANALYSIS_PT_shooting_distance(bpy.types.Panel):
    bl_label = "Shooting Distance"
    bl_idname = "ANALYSIS_PT_shooting_distance"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_shooting_distance_panel(self.layout, context.scene, context, utils.translate)
    
class ANALYSIS_PT_converter(bpy.types.Panel):
    bl_label = "Unit Converter"
    bl_idname = "ANALYSIS_PT_converter"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_converter_panel(self.layout, context.scene, context, utils.translate)

class ANALYSIS_PT_speedometer(bpy.types.Panel):
    bl_label = "Speedometer"
    bl_idname = "ANALYSIS_PT_speedometer"
    bl_space_type = 'VIEW_3D'; bl_region_type = 'UI'; bl_category = 'AnalysisToolkit'
    bl_options = {'DEFAULT_CLOSED'}
    def draw(self, context):
        draw_speedometer_panel(self.layout, context.scene, context, utils.translate)

# --- Registration List ---
classes = (
    ANALYSIS_PT_texeldensity,
    ANALYSIS_PT_luxmeter,
    ANALYSIS_PT_luxev,
    ANALYSIS_PT_ev,
    ANALYSIS_PT_horizon,
    ANALYSIS_PT_parallax,
    ANALYSIS_PT_shooting_distance,
    ANALYSIS_PT_converter,
    ANALYSIS_PT_speedometer,
)
